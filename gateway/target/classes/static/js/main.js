...
function initAccount(account) {
    user = new User(account.customerId, account.lastSeen, account.note);
...
function initGreetingPage() {
    ...
    $("#lefttitle").append(escape(user.login));
...
function greetingPageAgain() {
    ...
    $("#righttitle").append('<span class="bluetext"> Last seen: </span>').append(user.lastSeen);
    $("#lefttitle").append(escape(user.login)).append('<span class="bluetext"> is logged in</span>');
}
